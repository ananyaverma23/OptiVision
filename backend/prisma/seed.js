...
async function main() {
  ...
  for (const f of framesData) {
    const frame = await prisma.frame.create({ data: { storeId: store.id, ...f } });
    frames.push(frame);
  }
  ...
  for (const l of lensesData) {
    const lens = await prisma.lens.create({ data: { storeId: store.id, ...l } });
    lenses.push(lens);
  }
  ...
  for (const a of accData) await prisma.accessory.create({ data: { storeId: store.id, ...a } });
