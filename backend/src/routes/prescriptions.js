...
router.post('/', async (req, res, next) => {
  try {
    ...
    const rx = await prisma.prescription.create({ data: { customerId, doctorName, date: date ? new Date(date) : new Date(), rightSph: n(rightSph), rightCyl: n(rightCyl), rightAxis: n(rightAxis), rightAdd: n(rightAdd), rightPd: n(rightPd), leftSph: n(leftSph), leftCyl: n(leftCyl), leftAxis: n(leftAxis), leftAdd: n(leftAdd), leftPd: n(leftPd), pd: n(pd), imageUrl, notes } });
