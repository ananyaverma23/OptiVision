...
router.post('/', async (req, res, next) => {
  try {
    ...
    const lens = await prisma.lens.create({ data: { storeId: req.storeId, name, lensType: lensType||'SINGLE_VISION', lensIndex: lensIndex||'1.56', coating: coating||[], brand, purchasePrice: Number(purchasePrice)||0, sellingPrice: Number(sellingPrice), stockQty: Number(stockQty)||100, lowStockAlert: Number(lowStockAlert)||10, supplierId } });
